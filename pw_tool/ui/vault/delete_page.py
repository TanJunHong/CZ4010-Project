...
class DeletePage:
    def __init__(self, master):
        ...
        pw_tool.helper.db_helper.cursor.execute("SELECT website, username, password FROM password_vault")
        if pw_tool.helper.db_helper.cursor.fetchall() is not None:
            i = 0
            while True:
                pw_tool.helper.db_helper.cursor.execute("SELECT website, username,password FROM password_vault")
                array = pw_tool.helper.db_helper.cursor.fetchall()
                ...
                i = i + 1
                pw_tool.helper.db_helper.cursor.execute("SELECT website, username, password FROM password_vault")
                if len(pw_tool.helper.db_helper.cursor.fetchall()) <= i:
                    break
    ...
    def __delete_from_vault(self):
        pw_tool.helper.db_helper.cursor.execute("DELETE FROM password_vault WHERE id =?", input)
