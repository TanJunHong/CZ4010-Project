...
from passwordmanager.ui import registration_page, vault_page
...
class LoginPage:
    def __init__(self):
        ...
        self.__password_label = ttk.Label(master=self.__window, text="Enter Master Password", font=("Arial", 25),
                                          background="SystemButtonFace")
        ...
        self.__login_button = ttk.Button(master=self.__window, text="Login", style="TButton",
                                         command=self.__verify_login)
        self.__login_button.pack()

        self.__register_button = ttk.Button(master=self.__window, text="Register", style="TButton",
    ...
    def __show_register_page(self):
        gui_helper.clear_fields(window=self.__window)
        self.__window.withdraw()
        registration_page.RegistrationPage(master=self.__window)
    ...
    def __verify_login(self):
        ...
        if not password_hash or not context.verify(self.__password_entry.get(), password_hash[0]):
        ...
        self.__window.after(1000, lambda: vault_page.VaultPage(master=self.__window))
